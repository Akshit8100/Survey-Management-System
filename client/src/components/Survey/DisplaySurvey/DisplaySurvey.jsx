...
const DisplaySurvey = (props) => {
  ...
  const handleOptionChange = (selectedOptionValue, questionId) => {
  setSurvey((prevSurvey) => {
    const updatedSurvey = { ...prevSurvey };
    const questionIndex = updatedSurvey.questions.findIndex(
      (q) => q._id === questionId
    );

    if (questionIndex !== -1) {
      updatedSurvey.questions[questionIndex].response = {
        ...updatedSurvey.questions[questionIndex].response,
    ...
    const allInputsValid = updatedSurvey.questions
      .filter((question) => question.type !== "new section")
      .every((question) => question.response.response.length > 0);
    setIsFormValid(allInputsValid);
  ...
  const submitSurvey = async (e) => {
    ...
    try {
      const response = await fetch(
        `/api/surveys/update-responses/${survey._id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
